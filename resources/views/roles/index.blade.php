      ...
      <div class="bg-card text-card-foreground rounded-lg border shadow-sm">
        <div class="p-6">
          <div class="mb-6 flex items-center justify-between">
            <div class="space-y-1">
              <h3 class="text-2xl font-semibold leading-none tracking-tight">All Roles</h3>
              <p class="text-muted-foreground text-sm">Manage system roles and their permissions here.</p>
            </div>
          ...
          <div class="rounded-md border border-gray-200 px-4 py-6 pb-4">
            <div class="mb-6 flex flex-wrap gap-4">
              <form action="{{ route('roles.index') }}" method="GET" class="flex w-full flex-wrap gap-4">
                <div class="flex-grow">
            ...
            <table class="w-full caption-bottom text-sm">
              ...
              <tbody class="[&_tr:last-child]:border-0">
                @foreach ($roles as $role)
                  <tr class="hover:bg-muted/50 data-[state=selected]:bg-muted border-b transition-colors">
                    <td class="p-4 align-middle">{{ $role->name }}</td>
